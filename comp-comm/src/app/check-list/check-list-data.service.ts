...
export class CheckListDataService {
  ...
  getCheckedItemRatioText() {
    const curCheckedItemCnt = this.checkList.filter(i => i.isChecked).length;
    const totalCheckListCnt = this.checkList.length;
    const roundedRatio = Math.round((curCheckedItemCnt / totalCheckListCnt) * 100);
  ...
  private getNewCheckItem(idx: number) {
    return {
      idx: idx,
      content: this.getCheckListMsg(idx),
      isChecked: false
    };
  }
